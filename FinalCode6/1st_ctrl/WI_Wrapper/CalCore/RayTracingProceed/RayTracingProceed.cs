                ...
                for (int j = 0; j < rxs.Count; j++)
                {
                    //该接收区只有一个接收机
                        if (rxs[j].Count == 1)
                        {
                            ...
                            //文件输出
                            string path = ".\\.\\project\\area050502\\areaSituationResult.txt";
                            ...
                            for (int m = 0; m < reArea.areaSituationNodes.Count; m++)
                            {
                                if (reArea.areaSituationNodes[m].paths.Count != 0)
                                {
                                    ...
                                    string appendText2 =  reArea.areaSituationNodes[m].totleEfield.X.Re + "," + reArea.areaSituationNodes[m].totleEfield.X.Im + ", " + reArea.areaSituationNodes[m].totleEfield.Y.Re + "," + reArea.areaSituationNodes[m].totleEfield.Y.Im + ", " + reArea.areaSituationNodes[m].totleEfield.Z.Re + "," + reArea.areaSituationNodes[m].totleEfield.Z.Im + ",";
                                    sb.Append(appendText2);
                                    string appendText3 =   Math.Sqrt(Math.Pow(reArea.areaSituationNodes[m].totleEfield.X.Re, 2) + Math.Pow(reArea.areaSituationNodes[m].totleEfield.X.Im, 2) + Math.Pow(reArea.areaSituationNodes[m].totleEfield.Y.Re, 2) + Math.Pow(reArea.areaSituationNodes[m].totleEfield.Y.Im, 2) + Math.Pow(reArea.areaSituationNodes[m].totleEfield.Z.Re, 2) + Math.Pow(reArea.areaSituationNodes[m].totleEfield.Z.Im, 2)) +"\r\n";
                                    sb.Append(appendText3);
                                }
                                else
                                {
                                    string appendText1 =  m + "," + 0 + "，";
                                    sb.Append(appendText1);
                                    string appendText2 =  0 + "," + 0 + ", " + 0 + "," + 0 + ", " + 0 + "," + 0 + ",";
                                    sb.Append(appendText2);
                                    string appendText3 =  0 + "\r\n";
                                    sb.Append(appendText3);
                            ...
                            for (int m = 0; m < reArea.areaSituationNodes.Count; m++)
                            {
                                for (int n = 0; n < reArea.areaSituationNodes[m].classifiedFrequencyPaths.Count; n++)
                                {
                                    //P2mFileOutput.p2mfileoutputArea(reArea.areaSituationNodes[m].classifiedFrequencyPaths[n], DirectoryPath, ProName, txs[i].TxNum, txTotal, reArea, terpath, txindex, txFrequencyBand[m].MidPointFrequence, txFrequencyBand[m].FrequenceWidthMin, txFrequencyBand[m].FrequenceWidthMax);
                                }
                            }
                        ...
                        else
                            {
                                ...
                                index[0] = j;
                                RayTubeMethod rayTubeMethod = new RayTubeMethod(txs[i], rxs[j][0], newTer, buildings, kdAccel, 32);
