...
namespace CalculateModelClasses
{
    ...
    public class Path
    {
        ...
        /// <summary>
        /// 得到不经判断的路径节点信息，如果路径不存在返回null
        /// </summary>
        /// <returns>更新后的具有准确节点信息的路径</returns>
        public Path GetPossibleReversePaths()
        {
            if (this.node.Count > 2)//直射不需要进行反向
            {
                List<Point> pointsOfPath = this.GetPossiblePointsOfPath();
                if (pointsOfPath.Count == 0)
                {
                    return null;
                }
        ...
        private static AdjacentEdge GetNewDiffractEdge(Face triangle, AdjacentEdge diffractEdge, Point point)
        {
            ...
            Point point1 = diffractEdge.GetInfiniteDiffractionPoint(triangle.Vertices[0], point);
            Point point2 = diffractEdge.GetInfiniteDiffractionPoint(triangle.Vertices[1], point);
            Point point3 = diffractEdge.GetInfiniteDiffractionPoint(triangle.Vertices[2], point);
        ...
        public static List<Point> GetTwoDiffractionPoints(AdjacentEdge edge1, AdjacentEdge edge2, Point beginPoint, Point lastPoint)
        {
            ...
            RayInfo ray1 = edge2.getVectorFromEdge2EdgeWithAngle(p21, angle1, tempLine)[0];//因为tempLine和edge2时平行的，所以只会有一条returnRay默认为returnRay[0]
            RayInfo ray2 = edge2.getVectorFromEdge2EdgeWithAngle(p22, angle2, tempLine)[0];

            SpectVector vector1 = new SpectVector(lastPoint, ray1.Origin);
            SpectVector vector2 = new SpectVector(lastPoint, ray2.Origin);
            if (SpectVector.VectorDotMultiply(vector1, vector2) > 0)  //点乘大于0.表示在棱的外部
            {
                ray1 = edge2.getVectorFromEdge2EdgeWithAngle(p21, angle1, tempLine)[1];
                ray2 = edge2.getVectorFromEdge2EdgeWithAngle(p22, angle1, tempLine)[1];
            ...
            while (p1.GetDistance(p2) > 0.000001)
            {
                ...
                if (isFirst)
                {
                     tempRay1 = edge1.getVectorFromEdge2EdgeWithAngle(tempPoint, tempAngle1, edge2)[0];
                     tempAngle2 = SpectVector.VectorPhase(tempRay1.RayVector, edge2.LineVector);
                     tempRay2 = edge2.getVectorFromEdge2EdgeWithAngle(tempRay1.Origin, tempAngle2, tempLine)[0];
                     tempDistance = lastPoint.GetDistanceToLine(tempRay2.Origin, tempRay2.RayVector);
                }
                else if(edge1.getVectorFromEdge2EdgeWithAngle(tempPoint, tempAngle1, edge2).Count==2)
                {
                     tempRay1 = edge1.getVectorFromEdge2EdgeWithAngle(tempPoint, tempAngle1, edge2)[1];
                     tempAngle2 = SpectVector.VectorPhase(tempRay1.RayVector, edge2.LineVector);
                     tempRay2 = edge2.getVectorFromEdge2EdgeWithAngle(tempRay1.Origin, tempAngle2, tempLine)[0];
                     tempDistance = lastPoint.GetDistanceToLine(tempRay2.Origin, tempRay2.RayVector);
                }
            ...
            if (diffractPoints.Count == 0 && p1.GetDistance(p2) > 0.000001)
            {
                ...
                Point p23 = edge2.GetDiffractionPoint(p3, lastPoint);
        ...
        /// <param name="indexOfPath">所要计算的路径的下标</param>
        /// <returns>按路径经过顺序存储的各个点的坐标</returns>
        public List<Point> GetPossiblePointsOfPath()
        {
            List<Point> tempPointList = new List<Point>();
            int nodeIndexOfDiffractEdge1 = MaxNodeNumber, nodeIndexOfDiffractEdge2 = MaxNodeNumber;
            for (int i = 1; i < this.node.Count - 1; i++)
            ...
            if (nodeIndexOfDiffractEdge1 == MaxNodeNumber && nodeIndexOfDiffractEdge2 == MaxNodeNumber)
            {
                tempPointList = this.GetPossiblePointsOfOnlyReflectPath();//如果不存在绕射节点，进行全为反射的反向计算
            }
            else if (nodeIndexOfDiffractEdge1 != MaxNodeNumber && nodeIndexOfDiffractEdge2 == MaxNodeNumber)
            {
                tempPointList = this.GetPossiblePointsOfContainOneDiffractPath(nodeIndexOfDiffractEdge1);//如果仅存在一个绕射节点，进行含有一次绕射的反向计算
            }
            else if (nodeIndexOfDiffractEdge1 != MaxNodeNumber && nodeIndexOfDiffractEdge2 != MaxNodeNumber
                && Math.Abs(nodeIndexOfDiffractEdge2 - nodeIndexOfDiffractEdge1) == 1)
            {
                //如果存在两个绕射节点，且相邻，进行两次相邻绕射的反向计算
                tempPointList = this.GetPossiblePointsOfContainTwoAdjacentDiffractPath(nodeIndexOfDiffractEdge1, nodeIndexOfDiffractEdge2);
            }
            else if (nodeIndexOfDiffractEdge1 != MaxNodeNumber && nodeIndexOfDiffractEdge2 != MaxNodeNumber
                && Math.Abs(nodeIndexOfDiffractEdge2 - nodeIndexOfDiffractEdge1) != 1)
            {
                //如果存在两个绕射节点且不相邻，进行两次不相邻绕射的反向计算
                tempPointList = this.GetPossiblePointsOfContainTwoNoAdjacentDiffractPath(nodeIndexOfDiffractEdge1, nodeIndexOfDiffractEdge2);
            }
            return tempPointList;
        }
        ...
        /// </summary>
        /// <returns>按路径经过顺序存储的各个点的坐标</returns>
        private List<Point> GetPossiblePointsOfOnlyReflectPath()
        {
            List<Point> pointList = new List<Point>();
            Stack<Face> reflectTriangles = new Stack<Face>(), reflectTrianglesForMirror = new Stack<Face>();
            Stack<Point> mirrorPoints = new Stack<Point>(), frontPoints = new Stack<Point>();
            ...
            {
                RayInfo tempRay = new RayInfo(frontPoints.Peek(), mirrorPoints.Pop());
                if (tempRay.GetCrossPointWithInfiniteFace(reflectTrianglesForMirror.Peek()) != null)//若镜像点与前一交点连线与三角面所在平面有交点则将交点加入链表
                {
                    frontPoints.Push(tempRay.GetCrossPointWithInfiniteFace(reflectTrianglesForMirror.Pop()));
                    pointList.Add(frontPoints.Peek());
                }
                else
                {
        ...
        /// <param name="indexOfEdgeOfThisPath">绕射点所在的下标位置信息</param>
        /// <returns>按路径经过顺序存储的各个点的坐标</returns>
        private List<Point> GetPossiblePointsOfContainOneDiffractPath(int indexOfEdgeOfThisPath)
        {
            List<Point> pointList = new List<Point>();
            Stack<Face> reflectTrianglesBeforeDiffract = new Stack<Face>(), reflectTrianglesForMirrorBeforeDiffract = new Stack<Face>(),
                reflectTrianglesAfterDiffract = new Stack<Face>(), reflectTrianglesForMirrorAfterDiffract = new Stack<Face>();
            ...
            Point diffractPoint = this.node[indexOfEdgeOfThisPath].DiffractionEdge.GetInfiniteDiffractionPoint(mirrorPointsBeforeDiffract.Peek(), mirrorPointsAfterDiffract.Peek());
            if (diffractPoint == null)
            {
                return new List<Point>();
            }
            ...
            Stack<Point> tempPoints = new Stack<Point>();
            tempPoints.Push(diffractPoint);
            pointList = GetPossiblePointListOfOneOrTwoAdjacentDiffractPath(
                tempPoints, reflectTrianglesForMirrorBeforeDiffract, reflectTrianglesForMirrorAfterDiffract,
                mirrorPointsBeforeDiffract, mirrorPointsAfterDiffract);//得到路径上的点
        ...
        /// <param name="indexOfEdge2OfThisPath">第二次绕射节点所在下标位置信息</param>
        /// <returns>按路径经过顺序存储的各个点的坐标</returns>
        private List<Point> GetPossiblePointsOfContainTwoAdjacentDiffractPath(int indexOfEdge1OfThisPath, int indexOfEdge2OfThisPath)
        {
            List<Point> pointList = new List<Point>();
            Stack<Face> reflectTrianglesBeforeDiffract = new Stack<Face>(), reflectTrianglesForMirrorBeforeDiffract = new Stack<Face>(),
                reflectTrianglesAfterDiffract = new Stack<Face>(), reflectTrianglesForMirrorAfterDiffract = new Stack<Face>();
            ...
            List<Point> difffractPoints = GetPossibleTwoDiffractionPoints(
                this.node[indexOfEdge1OfThisPath].DiffractionEdge, this.node[indexOfEdge2OfThisPath].DiffractionEdge,
                mirrorPointsBeforeDiffract.Peek(), mirrorPointsAfterDiffract.Peek());//获得两个绕射点
            if (difffractPoints.Count == 0)
            {
        ...
        /// <param name="indexOfEdge2OfThisPath">第二次绕射节点所在下标位置信息</param>
        /// <returns>按路径经过顺序存储的各个点的坐标</returns>
        private List<Point> GetPossiblePointsOfContainTwoNoAdjacentDiffractPath(int indexOfEdge1OfThisPath, int indexOfEdge2OfThisPath)
        {
            List<Point> pointList = new List<Point>();
            for (int i = 0; i < this.node.Count - 2; i++)
            {
            ...
            List<Point> diffractPoints = GetPossibleTwoDiffractionPoints(mirrorDiffractEdge.Pop(), updatedDiffractEdge2,
                mirrorPointsBetweenDiffract.Pop(), mirrorPointsAfterDiffract.Peek());
            if (diffractPoints.Count == 0)
            {
                return new List<Point>();
        ...
        /// <param name="reflectTriangle">反射面</param>
        /// <returns></returns>
        private static Point GetPossibleReflectPoint(Point originPoint, Point targetPoint, Face reflectTriangle)
        {
            RayInfo rayTemp = new RayInfo(originPoint, targetPoint);
            if (rayTemp.GetCrossPointWithInfiniteFace(reflectTriangle) != null)
            {
                return rayTemp.GetCrossPointWithInfiniteFace(reflectTriangle);
            }
            else
            {
                return null;
        ...
        /// <param name="pointList"></param>
        /// <returns>顺序存储的各个点的坐标</returns>
        private static List<Point> GetPossiblePointListOfOneOrTwoAdjacentDiffractPath(
            Stack<Point> tempPoints, Stack<Face> beforeTriangles, Stack<Face> afterTiangles,
            Stack<Point> beforeMirror, Stack<Point> afterMirror)
        {
            List<Point> pointList = new List<Point>();
            ...
            {
                RayInfo tempRay = new RayInfo(tempPoints.Peek(), beforeMirror.Pop());
                if (tempRay.GetCrossPointWithInfiniteFace(beforeTriangles.Peek()) != null)
                {
                    tempPoints.Push(tempRay.GetCrossPointWithInfiniteFace(beforeTriangles.Pop()));
                }
                else
                {
                    return new List<Point>();
            ...
            {
                RayInfo tempRay = new RayInfo(pointList[pointList.Count - 1], afterMirror.Pop());
                if (tempRay.GetCrossPointWithInfiniteFace(afterTiangles.Peek()) != null)
                {
                    pointList.Add(tempRay.GetCrossPointWithInfiniteFace(afterTiangles.Pop()));
                }
                else
                {
                    return new List<Point>();
        ...
        /// <param name="endPoint">第二条绕射棱后的点</param>
        /// <returns>用List的形式返回两个点的坐标</returns>
        public static List<Point> GetPossibleTwoDiffractionPoints(AdjacentEdge edge1, AdjacentEdge edge2, Point beginPoint, Point lastPoint)
        {
            //求由第二条棱的两端点确定的第一条棱无限延长线上的两绕射点
            Point p1 = edge1.GetInfiniteDiffractionPoint(beginPoint, edge2.StartPoint);
            Point p2 = edge1.GetInfiniteDiffractionPoint(beginPoint, edge2.EndPoint);
            ...
            AdjacentEdge tempLine = new AdjacentEdge(
                new Point(-10000 * edge2.LineVector.a + lastPoint.X, -10000 * edge2.LineVector.b + lastPoint.Y, -10000 * edge2.LineVector.c + lastPoint.Z),
                new Point(10000 * edge2.LineVector.a + lastPoint.X, 10000 * edge2.LineVector.b + lastPoint.Y, 10000 * edge2.LineVector.c + lastPoint.Z),
                edge2.LineVector);//获得过点lastPoint与第二条棱平行共面的长线段
            double angle1 = SpectVector.VectorPhase(new SpectVector(p1, p21), edge2.LineVector);//得到第二条棱的入射角
            double angle2 = SpectVector.VectorPhase(new SpectVector(p2, p22), edge2.LineVector);
            RayInfo ray1 = edge2.getVectorFromEdge2EdgeWithAngle(p21, angle1, tempLine)[0];
            RayInfo ray2 = edge2.getVectorFromEdge2EdgeWithAngle(p22, angle2, tempLine)[0];
