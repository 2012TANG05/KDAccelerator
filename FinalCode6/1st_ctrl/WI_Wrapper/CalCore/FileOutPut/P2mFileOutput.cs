...
namespace FileOutput
{
    public class P2mFileOutput
    {
        ...
        public static void p2mfileoutputArea(List<CalculateModelClasses.Path> Paths, string filePath, string ProjName, int TxIndex, int txTotol, ReceiveArea rArea, string ter, int index, int Frequence, int minFrequence, int maxFrequence)
        {
            if (maxFrequence > 9999)
            { throw new Exception("频率过大"); }
            lock (typeof(P2mFileOutput))
            {
                ...
                {
                    Dictionary<int, List<Plural>> projectionResult = null;
                    //OutputDelegateAreaSituation output = PowerOutput.OutPathArea;
                    //output += ErmOutput.OutPathArea;
                    //output(Paths, filePath, ProjName, TxIndex, txTotol, rArea, ter, ref projectionResult, Frequence, minFrequence, maxFrequence);
                    projectionResult.Clear();
                    GC.Collect();
        ...
        private delegate void OutputDelegateAreaSituation(List<CalculateModelClasses.Path> Paths, string sPath, string projectName, int txIndex, int txTotol, ReceiveArea rArea, string ter, ref Dictionary<int, List<Plural>> projectionResult, int Frequence, int minFrequence, int maxFrequence);
